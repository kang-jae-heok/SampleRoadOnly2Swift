...
import Foundation
class OrderCouponCollectionCell: UICollectionViewCell {
    var getBtnTapped:(() -> Void)?
    var infoBtnTapped:(() -> Void)?
    static let cellId = "OrderCouponTableCellId"
    ...
    lazy var innerCircleView = UIView().then {
        $0.backgroundColor = common2.pointColor()
        $0.isHidden = true
    }
    lazy var circleView = UIView().then {
        $0.backgroundColor = .white
        $0.addSubview(innerCircleView)
